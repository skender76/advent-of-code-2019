...
func calculate( input_val int) {
	...
	for op_code_pos := 0; op_code_pos < len(input); op_code_pos +=nextOpCode{
		...
		if opcode == ADD {
			...
			nextOpCode = OFFSET_SAVE
		} else if opcode == OUT {
			var targetPos = calc_pos(input, op_code_pos+1, mode_first)
			fmt.Println("Out", input[targetPos])
			nextOpCode = OFFSET_OUT
		} else {
			fmt.Println("Wrong...")
			break
		}
...
func calc_pos(input [678]int, pos, mode int) int {
	var result_pos = 0
...
func calcMul(input [678]int, op_code_pos, mode_first, mode_second, out_mode int) (int, int) {
...
func calcSum(input [678]int, op_code_pos, mode_first, mode_second, out_mode int) (int, int) {
	var firstArgPos = calc_pos(input, op_code_pos+1, mode_first)
	var secondArgPos = calc_pos(input, op_code_pos+2, mode_second)
	var targetPos = calc_pos(input, op_code_pos+3, out_mode)
	var value = input[firstArgPos] + input[secondArgPos]
